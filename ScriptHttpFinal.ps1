...
function Instalar-IIS {
    Write-Output "Preparando todo..."
    Start-Sleep -Seconds 2  # Simular preparación
...
function Obtener-Versiones-Nginx {
    return @("nginx-1.24.0.zip", "nginx-1.23.3.zip")  # Solo las dos primeras versiones
}

function Obtener-Versiones-Apache {
    return @(
        "httpd-2.4.63-250207-win64-VS17.zip",
        "httpd-2.4.62-240101-win64-VS17.zip"  # Solo las dos primeras versiones
    )
}
...
function Instalar-Nginx {
    ...
    do {
        ...
        if ($seleccion -lt 1 -or $seleccion -gt $versiones.Length) {
            Write-Output "Error: Ingrese un número válido (1 o 2)."
        }
    } while ($seleccion -lt 1 -or $seleccion -gt $versiones.Length)
...
do {
    ...
    switch ($opcion) {
        ...
        default {
            Write-Output "Error: Ingrese un número válido (1, 2, 3 o 4)."
        }
    }
} while ($opcion -ne "4")
