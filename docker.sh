...
docker exec -it postgres2 bash -c "PGPASSWORD=$POSTGRES2_PASSWORD psql -U $POSTGRES2_USER -d $POSTGRES2_DB -c \"CREATE USER ruben WITH PASSWORD 'ruben2004';\""
docker exec -it postgres2 bash -c "PGPASSWORD=$POSTGRES2_PASSWORD psql -U $POSTGRES2_USER -d $POSTGRES2_DB -c \"GRANT ALL PRIVILEGES ON DATABASE $POSTGRES2_DB TO ruben;\""
docker exec -it postgres2 bash -c "PGPASSWORD=$POSTGRES2_PASSWORD psql -U $POSTGRES2_USER -d $POSTGRES2_DB -c \"GRANT ALL PRIVILEGES ON TABLE prueba_1 TO ruben;\""
...
docker exec -it postgres1 bash -c "PGPASSWORD=ruben2004 psql -h postgres2 -U ruben -d $POSTGRES2_DB -c 'SELECT * FROM prueba_1;'"

# Insertar datos desde postgres1 a postgres2
echo "Insertando datos desde postgres1 a la tabla en postgres2:"
docker exec -it postgres1 bash -c "PGPASSWORD=ruben2004 psql -h postgres2 -U ruben -d $POSTGRES2_DB -c \"INSERT INTO prueba_1 (nombre) VALUES ('Durant');\""
...
echo "- De postgres1 a postgres2: host: postgres2, usuario: ruben, contraseña: ruben2004, BD: $POSTGRES2_DB"
echo ""
